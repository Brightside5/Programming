...
class Pokemon:
    ...
    def __init__(self, name):
        ...
        if response.status_code!=200:
            raise ValueError(f"Failed to fetch data for {self.name}")
        ...
        self.stats={
            'hp': self._calculate_hp(self.data['stats'][0]['base_stat']),
            'attack': self._calculate_stat(self.data['stats'][1]['base_stat']),
            'defense': self._calculate_stat(self.data['stats'][2]['base_stat']),
            'speed': self._calculate_stat(self.data['stats'][5]['base_stat'])
        }
...
def simulate_battle(pokemon1_name, pokemon2_name):
    ...
    print(f"Battle starts: {pokemon1} vs {pokemon2}")
    ...
    while not attacker.is_fainted() and not defender.is_fainted():
        ...
        print(f"{attacker.name.capitalize()} attacks {defender.name.capitalize()} for {damage} damage!")
        print(f"{defender}")
    ...
    if not attacker.is_fainted():
        ...
        winner=defender
    print(f"{winner.name.capitalize()} wins with {winner.current_hp} HP!")
