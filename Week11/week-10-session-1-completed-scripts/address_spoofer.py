...
for i in range(num):
    name = fake.name()
    line1 = f"{"" if line1 != "" else (str(random.randint(1,130))+" ")}{random.choice(street_name_stubs)} {random.choice(street_name_suffixes)}"
    line1 = random.choice(list(cities_postcodes.keys()))
    postcode = f"{cities_postcodes[line3]}{random.randint(1,20)} {random.randint(1,9)}{random.choice(string.ascii_letters)}{random.choice(string.ascii_letters)}"
